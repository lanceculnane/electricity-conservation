...
full_pre = pd.read_csv("demographics.csv")
full_pre.fillna(0, inplace=True)
full = full_pre.ix[:66,1:]
...
# filled = imp.transform(full_pre.ix[:66,1:]) #transforms all NaNs to mean in each column
...
rf = RandomForestRegressor(n_estimators=1000,oob_score=True, max_features=7, n_jobs=3, verbose=True, min_samples_leaf=3, max_depth=20)
